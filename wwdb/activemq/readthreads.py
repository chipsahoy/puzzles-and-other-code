...
class MyListener(stomp.ConnectionListener):
    ...
    def on_message(self, headers, message):
        ...
        #print jsonmessage
        print('%s %s %s' %(jsonmessage['ThreadId'], datetime.datetime.utcnow(), jsonmessage['URL'][34:]))
#Check for a moved thread.
        ...
        if(cursor.rowcount == 0):
            cursor.execute("INSERT INTO Poster (posterid, postername) VALUES (%s, %s)",(int(jsonmessage['OP']['Id']), jsonmessage['OP']['Name']))
        ...
        #If not create the new poster and get their new id
        if(cursor.rowcount == 0):
            cursor.execute("INSERT INTO Poster (posterid, postername) VALUES (%s, %s)",(int(jsonmessage['LastPoster']['Id']), jsonmessage['LastPoster']['Name']))
        ...
        cursor.execute("SELECT * FROM thread2 WHERE threadid = %s",jsonmessage['ThreadId'])

        #If not, create it and flag for reading
        if(cursor.rowcount == 0):
            readThread = True
            if 'ThreadIconText' in jsonmessage:
                cursor.execute("INSERT INTO thread2 (threadid, OP, views, replies, URL, lastPoster, lastPostTime, title, iconText) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
					(int(jsonmessage['ThreadId']),jsonmessage['OP']['Id'],int(jsonmessage['Views']),int(jsonmessage['ReplyCount']),jsonmessage['URL'],
					jsonmessage['LastPoster']['Id'],jsonmessage['LastPostTime'],jsonmessage['Title'],jsonmessage['ThreadIconText']))
            else:
                cursor.execute("INSERT INTO thread2 (threadid, OP, views, replies, URL, lastPoster, lastPostTime, title) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",(int(jsonmessage['ThreadId']),jsonmessage['OP']['Id'],int(jsonmessage['Views']),int(jsonmessage['ReplyCount']),jsonmessage['URL'],jsonmessage['LastPoster']['Id'],jsonmessage['LastPostTime'],jsonmessage['Title']))
            ...
            if(int(row['views']) != int(jsonmessage['Views'])):
                cursor.execute("UPDATE thread2 SET views=%s WHERE threadid=%s", (int(jsonmessage['Views']), int(jsonmessage['ThreadId'])))
            if(int(row['replies']) != int(jsonmessage['ReplyCount']) or row['lastposttime'] != jsonmessage['LastPostTime']):
                cursor.execute("UPDATE thread2 SET replies=%s, lastPoster=%s, lastPostTime=%s WHERE threadid=%s",(int(jsonmessage['ReplyCount']), jsonmessage['LastPoster']['Id'], jsonmessage['LastPostTime'], int(jsonmessage['ThreadId'])))
                readThread=True
        ...
        #    conn.send(message="read thread!", destination=readthreads, headers={'expires': milliseconds, 'URL':jsonmessage['URL'],'threadid':int(jsonmessage['ThreadId']),'startPost':posts+1, 'CurrentUTC':timerow['curtime'] + " GMT"}, ack='auto')
