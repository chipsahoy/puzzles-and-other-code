...
#		print jsonmessage
		...
		cursor.execute("SELECT postid FROM Post WHERE postid = %s",int(jsonmessage['PostId']))
		if(cursor.rowcount == 0):
			cursor.execute("INSERT INTO Post (postid, postnumber, threadid, posterid, content, postTime, title) VALUES (%s,%s,%s,%s,%s,%s,%s)",(int(jsonmessage['PostId']), int(jsonmessage['PostNumber']),int(jsonmessage['ThreadId']), int(jsonmessage['Poster']['Id']), jsonmessage['Content'].encode('utf-8'), jsonmessage['Time'], jsonmessage['Title'].encode('utf-8')))
...
#			result = cursor.fetchone()
			...
			cursor.execute("SELECT * FROM Post, Edit, Poster WHERE Post.postid = Edit.postid AND Post.content=%s AND Post.postid=%s AND Edit.reason=%s AND Edit.edittime=%s AND Edit.editor = Poster.posterid AND Poster.posterid=%s",(jsonmessage['Content'].encode('utf-8'), int(jsonmessage['PostId']), jsonmessage['Edit']['Reason'].encode('utf-8'), edittime, jsonmessage['Edit']['Who']))
			if(cursor.rowcount == 0): #check to see if current edit stored
				cursor.execute("SELECT content FROM Post WHERE postid=%s", int(jsonmessage['PostId']))
				result = cursor.fetchone()
			...
			cursor.execute("UPDATE Post SET content=%s WHERE postid=%s", (jsonmessage['Content'].encode('utf-8'), int(jsonmessage['PostId'])))
		cursor.execute("UPDATE Post SET postnumber=%s, content=%s, title=%s WHERE postid=%s",(int(jsonmessage['PostNumber']), jsonmessage['Content'].encode('utf-8'), jsonmessage['Title'].encode('utf-8'), int(jsonmessage['PostId'])))
